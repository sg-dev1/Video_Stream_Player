...
void DemuxVideoThread::Impl::Run() {
  ...
  INFO_PRINT(***REMOVED***Starting demux video thread...***REMOVED***);
  bool ret;
  ...
  INFO_PRINT(***REMOVED***Finishing demux video thread***REMOVED***);
}
...
bool DemuxVideoThread::Impl::ProcessSampleDataInPlace(uint8_t *inoutData,
  ...
  if (stream_->GetNaluLengthSize() != 4) {
    ERROR_PRINT(***REMOVED***This algorithm only works for nalu length == 4***REMOVED***);
    return false;
  }
  ...
  while (inoutData < inoutDataEnd) {
    // PRINT_BYTE_ARRAY(***REMOVED***inoutData[*]***REMOVED***, inoutData, 4);
    uint32_t nalsize = 0;
  ...
  if (inoutData != inoutDataEnd) {
    ERROR_PRINT(***REMOVED***NAL unit definition incomplete inoutData != inoutDataEnd: ***REMOVED***
...
bool DemuxVideoThread::Impl::ProcessSample(uint8_t *inputData, uint32_t inputDataSize,
  ...
  if (stream_->GetNaluLengthSize() > 4) {
    ERROR_PRINT(***REMOVED***NALU length > 4 not supported.***REMOVED***);
    return false;
  } else if (stream_->GetNaluLengthSize() <= 0) {
    ERROR_PRINT(***REMOVED***NALU length must be > 0.***REMOVED***);
    return false;
  }

  while (inputData < inputDataEnd) {
    uint32_t nalsize = 0;
    PRINT_BYTE_ARRAY(***REMOVED***inputData[*]***REMOVED***, inputData, 430);
    for (unsigned int i = 0; i < stream_->GetNaluLengthSize(); i++) {
      nalsize = (nalsize << 8) + *inputData++;
    }
    DEBUG_PRINT(***REMOVED***Using nalsize: ***REMOVED*** << nalsize);
    DEBUG_PRINT(***REMOVED***Using nalunitsum: ***REMOVED*** << nalunitsum);
    ...
    if (outputData + 4 + nalsize + stream_->GetVideoExtraDataSize() >
      ...
      ERROR_PRINT(***REMOVED***Too small output data array given. Would get out of range ***REMOVED***
                      ***REMOVED***in next iteration.***REMOVED***);
      return false;
    }
    ...
    if (stream_->GetVideoExtraData() &&
      ...
      DEBUG_PRINT(***REMOVED***Copying sps/pps data of size ***REMOVED***
    ...
    if (nalsize + stream_->GetNaluLengthSize() + nalunitsum >
        *bytes_of_cleartext_data + *bytes_of_encrypted_data) {
      ERROR_PRINT(
          ***REMOVED***NAL Unit exceeds subsample definition (nalu length size: ***REMOVED***
              << static_cast<unsigned int>(stream_->GetNaluLengthSize()) << ***REMOVED***) ***REMOVED***
              << static_cast<unsigned int>(nalsize + stream_->GetNaluLengthSize() +
                                           nalunitsum)
              << ***REMOVED*** -> ***REMOVED*** << (*bytes_of_cleartext_data + *bytes_of_encrypted_data)
              << ***REMOVED*** ***REMOVED***);
      return false;
    } else if (nalsize + stream_->GetNaluLengthSize() + nalunitsum ==
               *bytes_of_cleartext_data + *bytes_of_encrypted_data) {
      DEBUG_PRINT(***REMOVED***advancing to next subsample***REMOVED***);
      bytes_of_cleartext_data++;
      bytes_of_encrypted_data++;
      subsampleCount--;
      nalunitsum = 0;
    } else {
      DEBUG_PRINT(***REMOVED***staying at current subsample***REMOVED***);
      nalunitsum = nalsize + stream_->GetNaluLengthSize();
    }
  }

  if (inputData != inputDataEnd || subsampleCount) {
    ERROR_PRINT(***REMOVED***NAL Unit definition incomplete (nalu length size: ***REMOVED***
                    << static_cast<unsigned int>(stream_->GetNaluLengthSize())
                    << ***REMOVED***) ***REMOVED*** << static_cast<unsigned int>(inputData - inputDataEnd)
                    << ***REMOVED*** -> ***REMOVED*** << subsampleCount << ***REMOVED*** ***REMOVED***);
    return false;
  }
...
void DecryptVideoThread::Impl::Run() {
  ...
  INFO_PRINT(***REMOVED***Starting decrypt thread...***REMOVED***);
  ...
  while (isRunning_) {
    ...
    DEBUG_PRINT(***REMOVED***dataToDecryptSize: ***REMOVED*** << dataToDecryptSize);
    ...
    if (errors > 0) {
      INFO_PRINT(***REMOVED***CDM DecryptAndDecodeFrame produced ***REMOVED***
                     << errors << ***REMOVED*** kNeedMoreData errors.***REMOVED***);
    }
  }

  INFO_PRINT(***REMOVED***Finishing decrypt thread***REMOVED***);
}
...
int DecryptVideoThread::Impl::ProcessFrame(size_t pos, std::unique_ptr<DEMUXED_SAMPLE> &sample,
  ...
  *dataToDecryptSize = _dataToDecryptSize;
  ...
  // INFO_PRINT(***REMOVED***Processing sample # ***REMOVED*** << pos);
  ...
  if (extraDataSize > 0) {
    ...
    DEBUG_PRINT(***REMOVED***Updated clear bytes of first subsample to ***REMOVED***
  ...
  if (ret == cdm::Status::kSuccess) {
    DEBUG_PRINT(***REMOVED***Got new decrypted frame!***REMOVED***);
    ...
    if (frameToSave.encoding == VIDEO_FORMAT_ENUM::kYv12 &&
      ...
      WARN_PRINT(***REMOVED***Widevine seemed to set endcoding wrongly to Yv12***REMOVED***);
      frameToSave.encoding = VIDEO_FORMAT_ENUM::kI420;
    }
    ...
    ERROR_PRINT(***REMOVED***CDM DecryptAndDecodeFrame failed with ***REMOVED*** << ret);
    result = -1;
  } else {
    result = 1;
  }
...
void VideoThread::Impl::Run() {
  ...
  INFO_PRINT(***REMOVED***Starting render video thread...***REMOVED***);
  ...
  INFO_PRINT(***REMOVED***Finishing render video thread***REMOVED***);
}
