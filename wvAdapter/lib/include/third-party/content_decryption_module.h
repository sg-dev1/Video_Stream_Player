...
//
// src:
// https://hg.mozilla.org/mozilla-central/raw-file/6b98953213350d847f2d1c8165922af47b96eee4/dom/media/gmp/widevine-adapter/content_decryption_module.h

#ifndef CDM_CONTENT_DECRYPTION_MODULE_H_
#define CDM_CONTENT_DECRYPTION_MODULE_H_

// fix Stefan: NULL not declared
#include <cstddef>
...
// object.
CDM_EXPORT void* CreateCdmInstance(int cdm_interface_version,
...
struct AudioDecoderConfig {
  enum AudioCodec { kUnknownAudioCodec = 0, kCodecVorbis, kCodecAac };
...
struct VideoDecoderConfig {
  enum VideoCodec { kUnknownVideoCodec = 0, kCodecVp8, kCodecH264, kCodecVp9 };
...
enum StreamType { kStreamTypeAudio = 0, kStreamTypeVideo = 1 };
...
enum OutputProtectionMethods { kProtectionNone = 0, kProtectionHDCP = 1 << 0 };
...
enum QueryResult { kQuerySucceeded = 0, kQueryFailed };
...
enum InitDataType { kCenc = 0, kKeyIds = 1, kWebM = 2 };
...
class FileIOClient {
 public:
  enum Status { kSuccess = 0, kInUse, kError };
...
class Host_7 {
 public:
  ...
  virtual void OnRejectPromise(uint32_t promise_id, Error error,
  ...
  virtual void OnLegacySessionError(const char* session_id,
                                    uint32_t session_id_length, Error error,
                                    uint32_t system_code,
